...
void insertValue(DynArray* pStr) {
    ...
    if ((pStr->size)<=(pStr->nEl) && flg1==-1) {  //if is  not  present and array is full
        pStr->size = pStr->size + pStr->incSize;
        pStr->array = (int *) realloc(pStr->array, pStr->size);
        if (pStr->array == NULL) {
            printf("error in memory of new array\n");
            exit(1);
        }
    }
